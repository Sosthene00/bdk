...
use crate::{Error as WalletError, LocalUtxo, TransactionDetails, types};
...
pub struct Bip47Wallet<'w, D>
{
    ...
    main_wallet: &'w mut Wallet<D>,
    notification_wallet: &'w Wallet<D>,
    inbound_wallets: HashMap<PaymentCode, BTreeMap<u32, Option<Wallet<D>>>>,
    outbound_wallets: HashMap<PaymentCode, BTreeMap<u32, Option<Wallet<D>>>>,
    outbound_txs: HashMap<Script, Txid>,
}

impl<'w, D> Bip47Wallet<'w, D>
where
    D: bdk_chain::keychain::PersistBackend<types::KeychainKind, bdk_chain::ConfirmationTime>
{
    pub fn new<S>(
        seed: S,
        wallet: &'w mut Wallet<D>,
        notification_wallet: &'w Wallet<D>,
    ) -> Result<Self, Error>
    where
        S: Clone + DerivableKey<Legacy>,
    {
        ...
        Ok(Bip47Wallet {
            seed: bip47_seed,
            notification_wallet: notification_wallet,
            main_wallet: wallet,
    ...
    fn secret<Q: AsRef<[bip32::ChildNumber]>>(&self, derivation: &Q) -> SecretKey {
        let derived = self
            .seed
    ...
    pub fn build_notification_tx(
        &'w mut self,
        payment_code: &PaymentCode,
        initial_builder: TxBuilder<'w, D, DefaultCoinSelectionAlgorithm, CreateTx>,
        amount: Option<u64>,
        secp: SecpCtx,
        network: Network,
        ...
        let mut build_tx = |data| {
            let mut builder: TxBuilder<'w, D, DefaultCoinSelectionAlgorithm, CreateTx> = initial_builder
                .clone();
            builder
        ...
        let data_payload = self.payment_code().encode_blinded(blinding_factor);
        let mut builder = build_tx(&data_payload);
        builder
    ...
    pub fn get_payment_address(&mut self, payment_code: &PaymentCode) -> Result<Address, Error> {
        match self.outbound_wallets.get_mut(payment_code) {
            Some(wallets) => match wallets.values_mut().last() {
                Some(w) => Ok(w
                    .as_mut()
    ...
    //     for tx in self.notification_wallet.transactions()? {
...
impl From<DescriptorError> for Error {
    fn from(e: DescriptorError) -> Error {
        Error::DescriptorError(e)
    }
}
...
mod test {
    ...
    fn test_derive_outbound_wallet() {
        ...
        .unwrap();
        let alice_main_wallet = Wallet::new_no_persist(
            Bip44(m.clone(), KeychainKind::External),
            None,
            Network::Bitcoin,
        )
        .unwrap();
        let alice_notification_wallet = Wallet::new_no_persist(
            Bip47Notification(m.clone()),
            None,
            Network::Bitcoin,
        )
        .unwrap();
        let mut alice = Bip47Wallet::new(m, &mut alice_main_wallet, &alice_notification_wallet).unwrap();
        for i in 0..5 {
            let outbound_wallet = alice.derive_outbound_wallet(&bob_paymentcode, i).unwrap().unwrap();

            let bob_address = outbound_wallet.get_address(AddressIndex::New).to_string();
            let test_address = bob_addresses.pop().unwrap();
    ...
    fn test_derive_inbound_wallet() {
        ...
        .unwrap();
        let bob_main_wallet = Wallet::new_no_persist(
            Bip44(m.clone(), KeychainKind::External),
            None,
            Network::Bitcoin,
        )
        .unwrap();
        let bob_notification_wallet = Wallet::new_no_persist(
            Bip47Notification(m.clone()),
            None,
            Network::Bitcoin,
        )
        .unwrap();
        let mut bob = Bip47Wallet::new(m, &mut bob_main_wallet, &bob_notification_wallet).unwrap();
        for i in 0..5 {
            let inbound_wallet = bob.derive_inbound_wallet(&alice_paymentcode, i).unwrap().unwrap();

            let bob_address = inbound_wallet.get_address(AddressIndex::New).to_string();
            let test_address = bob_addresses.pop().unwrap();
            println!("Testing for address {}", test_address);
            assert_eq!(&bob_address, &test_address, "{} != {}", bob_address, test_address);
        }
